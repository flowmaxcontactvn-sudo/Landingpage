...
begin
  ...
  do update set
    ...
    so_phien_cuon_25 = heatmap_thiet_bi.so_phien_cuon_25 + case when p_max_scroll >= 25 then 1 else 0 end,
    so_phien_cuon_50 = heatmap_thiet_bi.so_phien_cuon_50 + case when p_max_scroll >= 50 then 1 else 0 end,
    so_phien_cuon_75 = heatmap_thiet_bi.so_phien_cuon_75 + case when p_max_scroll >= 75 then 1 else 0 end,
    so_phien_cuon_100 = heatmap_thiet_bi.so_phien_cuon_100 + case when p_max_scroll >= 100 then 1 else 0 end,
    so_phien_bo_form = heatmap_thiet_bi.so_phien_bo_form + case when p_form_abandoned then 1 else 0 end,
    cap_nhat_luc = now();
end;
$$;
